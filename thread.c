...
void mysqlsync_thread()
{
   ...
   if(!res){
	...
	printf("%s\n","unable connect to mysql database!");
        return 0;
   }
   ...
   item **llo_item=glo_item;
   int *llo_item_cmd=glo_item_cmd;  
   llo_item=llo_item+glo_item_head;
   ...
   if ( glo_item_head<=glo_item_tail){
	...
	glo_item_head++;
     ...
     if(*llo_item_cmd != NREAD_ADD){
        ...
        char deletesql[]="delete from cache_data where cache_key=\'";
        char *delete_str=malloc(strlen(deletesql) + (*llo_item)->nkey + 1);

        if( !delete_str ){
	   printf("%s\n","delete_str malloc error");
           return 0;	
        }
	...
	free(delete_str);
     ...
     char insertsql[]="insert into cache_data(cache_key,cache_data)values(\'";
   
     int sql_data_len=(*llo_item)->nbytes+(*llo_item)->nkey+strlen(insertsql)+20; 
     char* insert_str=malloc(sql_data_len);
     if( !insert_str ){
        
        printf("%s\n","insert_str malloc error");
        return 0; 
		
     }


     memset(insert_str,'\0',sql_data_len);


     strcat(insert_str,insertsql);
     sprintf(insert_str+strlen(insertsql),"%s",ITEM_key(*llo_item));
     strcat(insert_str,"\',");
     strcat(insert_str,"\'");
     strcat(insert_str,ITEM_data(*llo_item));

	     
     insert_str[strlen(insert_str)-2]='\0';
     strcat(insert_str,"\');");
