...
int main (int argc, char **argv) {
    ...
    #ifdef ENABLE_MYSQLSYNC    
    //setup mysqlsync thread 
     glo_item = malloc(max_mysql_sync_item*sizeof(item *));
     glo_item_cmd = malloc(max_mysql_sync_item*sizeof(item *)); 
     if (glo_item && glo_item_cmd) {
         memset(glo_item,0,max_mysql_sync_item);
	 mysqlsync();	
     }else{
         printf("%s\n","glo_item,glo_item_cmd malloc error!");
     }
    #endif  
...
void add_mysql_sync(item *itemadd,int itemcmd)
{ 
   if(itemadd->nkey > strlen(mysql_sync_mark))
   {
	if(!strncmp(ITEM_key(itemadd),mysql_sync_mark,strlen(mysql_sync_mark)))
         {	
		*(glo_item+glo_item_tail)=itemadd;
		*(glo_item_cmd+glo_item_tail)=itemcmd;
                 glo_item_tail++;
                 if(glo_item_tail+1>max_mysql_sync_item)glo_item_tail=0; 
